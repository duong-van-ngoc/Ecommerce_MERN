...
const AIChatBubble = () => {
    ...
    const [messages, setMessages] = useState(() => {
        const saved = localStorage.getItem('tobi_chat_history');
    ...
    const [sessionId] = useState(() => {
        const saved = localStorage.getItem('tobi_session_id');
        if (saved) return saved;
        const newId = Math.random().toString(36).substring(7);
        localStorage.setItem('tobi_session_id', newId);
        return newId;
    });
    ...
    useEffect(() => {
        if (messages.length > 0) {
            localStorage.setItem('tobi_chat_history', JSON.stringify(messages));
        }
    }, [messages]);
    ...
    const clearChat = () => {
        if (window.confirm("Bạn có muốn xóa toàn bộ lịch sử trò chuyện với Tobi không?")) {
            localStorage.removeItem('tobi_chat_history');
            localStorage.removeItem('tobi_session_id');
            setMessages([]);
            // Force generate new session ID if needed on next message
        }
    };
